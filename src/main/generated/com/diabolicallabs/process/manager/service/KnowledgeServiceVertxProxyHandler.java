...
*/
public class KnowledgeServiceVertxProxyHandler extends ProxyHandler {
  ...
  public KnowledgeServiceVertxProxyHandler(Vertx vertx, KnowledgeService service) {
    this(vertx, service, DEFAULT_CONNECTION_TIMEOUT);
  }

  public KnowledgeServiceVertxProxyHandler(Vertx vertx, KnowledgeService service, long timeoutInSecond) {
        ...
        case "getProcessService": {
          service.getProcessService(res -> {
            if (res.failed()) {
              ...
              ProxyHelper.registerService(ProcessService.class, vertx, res.result(), proxyAddress, false, timeoutSeconds);
        ...
        case "getRuleService": {
          service.getRuleService(res -> {
            if (res.failed()) {
              ...
              String proxyAddress = UUID.randomUUID().toString();
              ProxyHelper.registerService(RuleService.class, vertx, res.result(), proxyAddress, false, timeoutSeconds);
              msg.reply(null, new DeliveryOptions().addHeader("proxyaddr", proxyAddress));
            }
          });
          break;
        ...
        case "getTaskService": {
          service.getTaskService(res -> {
            if (res.failed()) {
              ...
              String proxyAddress = UUID.randomUUID().toString();
              ProxyHelper.registerService(TaskService.class, vertx, res.result(), proxyAddress, false, timeoutSeconds);
              msg.reply(null, new DeliveryOptions().addHeader("proxyaddr", proxyAddress));
            }
          });
          break;
